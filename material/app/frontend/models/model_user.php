...
class Model_User extends Model{
	public function check_account($login){
		return $this->select("SELECT id FROM `users` WHERE login=?", array($login));
	}
	...
	public function add_user($name,$login,$password,$access,$role,$image_url){
		$date_now = date('Y-m-d H:i:s', strtotime('+5 hours'));
		return $this->insert_getId("INSERT INTO `user` SET name=?,login=?,password=?,access=?,role=?,date_add=?,image_url", 
									array($name,$login,$password,$access,$role,$date_now,$image_url));
	}
	public function edit_user($user_id,$name,$login,$password,$access,$role,$image_url){
		$date_now = date('Y-m-d H:i:s', strtotime('+5 hours'));
